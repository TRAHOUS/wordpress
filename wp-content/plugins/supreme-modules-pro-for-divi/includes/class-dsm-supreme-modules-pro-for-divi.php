...
class Dsm_Supreme_Modules_Pro_For_Divi {
	...
	private function define_admin_hooks() {
		...
		// add_filter( 'plugin_auto_update_setting_html',  array( $this, 'dsm_auto_update_setting_html' ), 10, 3 );
	...
	public function dsm_admin_footer_text( $footer_text ) {
		...
		if ( $is_divi_supreme_screen ) {
			$footer_text = sprintf(
				...
				'<a href="https://wordpress.org/support/plugin/supreme-modules-for-divi/reviews/?rate=5#new-post" target="_blank" class="dsm-rating-link" data-rated="' . esc_attr__( 'Thanks :)', 'dsm-supreme-modules-pro-for-divi' ) . '">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'
			);
		}
	...
	public function dsm_attachment_fields_to_edit( $fields, $post ) {
		...
		$fields['dsm_upload_gallery_link_url_target'] = array(
			...
			/*
		$link_rel_value = get_post_meta( $post->ID, '_dsm_upload_gallery_link_rel', true );
		$fields['dsm_upload_gallery_link_rel'] = array(
			'label' => __( 'Link Rel', 'gallery-custom-links' ),
			'input' => 'html',
			'html'  => '
				<select class="widefat" name="attachments[' . $post->ID . '][dsm_upload_gallery_link_rel]" id="attachments[' . $post->ID . '][dsm_upload_gallery_link_rel]">
					<option value=""' . ( $link_rel_value == '' ? ' selected="selected"' : '' ) . '>' .
						__( 'None', 'dsm-supreme-modules-pro-for-divi' ) .
					'</option>
					<option value="nofollow"' . ( $link_rel_value == 'nofollow' ? ' selected="selected"' : '' ) . '>' .
						__( 'No Follow', 'dsm-supreme-modules-pro-for-divi' ) .
					'</option>
				</select>'
			);
			*/
