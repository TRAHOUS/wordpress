...
class IWP_MMB_Backup_Core {
	 ...
	 * @returns void
	 */
	public function logfile_open($nonce, $writeMode = false) {
...
*/
	...
	public function backupable_file_entities_final($arr, $full_info) {
		...
		if (stristr($os_name, 'windows')) {
			$windows_normalized_abspath = $path.'wp-admin/../';
			if (@opendir($windows_normalized_abspath)) {
				$path = $windows_normalized_abspath;
			}
		}
		if (is_array($path)) {
				...
				if (class_exists($call_method)) {
					...
					if ($remote_storage->supports_feature('multi_options')) {
						...
						if (!empty($settings)) {
							...
							$storage_objects_and_ids[$method]['instance_settings'] = $settings;
	...
	public function do_iwp_download_backup($params = array()) {
		...
		if ($next_call === true) {
			$iwp_backup_core->logfile_open($iwp_backup_core->nonce);
		}else{
			$iwp_backup_core->logfile_open($iwp_backup_core->nonce, true);
		}
